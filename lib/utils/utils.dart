...
class Utils {
  ...
  // iconFont 组件
  static Icon iconFont(int icon, [Color color = Colors.black45, double size = 15]) {
    return Icon(IconData(icon, fontFamily: 'iconfont'), color: color, size: size);
  }
  ...
  static Future<String> getUserId() async {
    return await StorageUtil.getStringItem('uid') ?? '';
  }
  ...
  // 选择弹窗
  static Future openAlert(String content, BuildContext context, {String title}) async {
    var action = await showDialog(context: context, builder: (BuildContext context){
      return AlertDialog(
